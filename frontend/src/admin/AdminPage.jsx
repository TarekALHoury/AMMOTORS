...
import { ArrowLeft, ArrowUpDown, BadgeCheck, CalendarDays, CarFront, Check, ChevronDown, ChevronRight, CircleDollarSign, Eye, FileText, Fuel, Gauge, ImageUp, LayoutDashboard, LoaderCircle, LogOut, Menu, Milestone, Palette, Plus, Search, SlidersHorizontal, SquarePen, Tags, Trash2, X } from 'lucide-react';
  ...
  return (
    <div className="admin-view">
      ...
      <section className="admin-toolbar" aria-label="Inventory controls">
        ...
        <SelectField label="Filter by status" name="status" value={filters.status} onChange={(event) => updateFilter('status', event.target.value)} options={['all', 'available', 'reserved', 'sold']} optionLabels={{ all: 'All statuses', available: 'Available', reserved: 'Reserved', sold: 'Sold' }} leadingIcon={BadgeCheck} hideLabel searchable={false} />
        <SelectField label="Sort inventory" name="sort" value={sort} onChange={(event) => setSort(event.target.value)} options={['newest', 'oldest', 'price-high', 'price-low', 'mileage-low', 'mileage-high', 'make']} optionLabels={{ newest: 'Newest year', oldest: 'Oldest year', 'price-high': 'Price: high to low', 'price-low': 'Price: low to high', 'mileage-low': 'Mileage: low to high', 'mileage-high': 'Mileage: high to low', make: 'Make and model' }} leadingIcon={ArrowUpDown} hideLabel searchable={false} />
      ...
      <details className="admin-filter-panel">
        ...
        <div className="admin-filter-grid">
          ...
          <SelectField label="Engine" ariaLabel="Filter by engine" name="engineFilter" value={filters.engine} onChange={(event) => updateFilter('engine', event.target.value)} options={['all', ...options.engines]} optionLabels={{ all: 'All engines' }} leadingIconSrc={engineIcon} leadingIconName="engine" searchable={false} />
          ...
          <SelectField label="Interior color" ariaLabel="Filter by interior color" name="interiorColorFilter" value={filters.interiorColor} onChange={(event) => updateFilter('interiorColor', event.target.value)} options={['all', ...options.interiorColors]} optionLabels={{ all: 'All interior colors' }} leadingIcon={Palette} searchable={false} />
          ...
          <label><span className="admin-filter-label"><Milestone size={16} aria-hidden="true" /> Maximum mileage</span><input aria-label="Maximum mileage" type="number" min="0" inputMode="numeric" value={filters.maxMileage} onChange={(event) => updateFilter('maxMileage', event.target.value)} placeholder="No maximum" /></label>
      ...
      ) : <div className="admin-inventory-grid">{filtered.map((car) => <article className="admin-inventory-card" data-tilt="7" key={car.id}><VehicleImage src={car.images?.[0]} alt={`${car.make} ${car.model}`} /><div><span className={`admin-status status-${car.status}`}>{car.status}</span><h2>{car.make} {car.model}</h2><p>{car.year} · {Number(car.mileage || 0).toLocaleString()} km</p><strong>{money(car.price)}</strong><div className="admin-card-actions"><button onClick={() => onNavigate('details', car)}>View</button><button onClick={() => onNavigate('edit', car)}>Edit</button><button className="danger" onClick={() => onDelete(car)}>Delete</button></div></div></article>)}</div>}
    </div>
  );
}
...
function Field({ label, name, value, error, onChange, type = 'text', leadingIcon: LeadingIcon, ...props }) {
  const errorId = `${name}-error`;
  return <label className={`admin-field ${error ? 'has-error' : ''}`}><span>{label}</span><div className={LeadingIcon ? 'admin-input-with-icon' : undefined}>{LeadingIcon && <LeadingIcon size={18} aria-hidden="true" />}<input name={name} type={type} value={value} onChange={onChange} aria-label={label} aria-invalid={Boolean(error)} aria-describedby={error ? errorId : undefined} {...props} /></div>{error && <small id={errorId} className="admin-field-error">{error}</small>}</label>;
}

function SelectField({ label, ariaLabel = label, name, value, error, onChange, options, optionLabels = {}, placeholder = 'Select an option', disabled = false, searchable = name === 'make', hideLabel = false, leadingIcon: LeadingIcon, leadingIconSrc, leadingIconName }) {
