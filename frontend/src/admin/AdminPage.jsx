...
function CarForm({ mode, initialCar, onCancel, onSave }) {
  ...
  async function submit(event) {
    ...
    setSaving(true);
    try {
      await onSave({ ...car, images: car.images.filter((image) => !image.startsWith('blob:')), year: Number(car.year), price: Number(car.price), mileage: Number(car.mileage), horsepower: Number(car.horsepower) }, selectedFiles.map((item) => item.file));
    } catch {
...
function AdminWorkspace({ email, onSignOut }) {
  ...
  async function saveCar(car, files) {
    if (view === 'add') {
      ...
      const updated = await updateAdminCar(selectedCar.id, car, files);
